...
function reload_img () {
  if(!halted) mjpeg_img.src = "http://" + location.host.substr(0, location.host.length - 5) + ":5050/cam_pic.php?time=" + new Date().getTime() + "&pDelay=" + preview_delay;
  else setTimeout("reload_img()", 500);
}

function error_img () {
  setTimeout("mjpeg_img.src = 'http://' + location.host.substr(0, location.host.length - 5) + ':5050/cam_pic.php?time=' + new Date().getTime();", 100);
}
...
function reload_ajax (last) {
  ajax_status.open("GET", "http://" + location.host.substr(0, location.host.length - 5) + ":5050/status_mjpeg.php?last=" + last,true);
  ajax_status.send();
}
